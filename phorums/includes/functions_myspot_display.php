<?php
	...
	function display_forums($root_data = '', $display_moderators = true, $return_moderators = false) {
		...
		//$dir = "phorums/";	// CC Removed 12/30/10
		...
		foreach ($forum_rows as $row) {
			...
			if ($row['parent_id'] == $root_data['forum_id'] && $row['forum_type'] == FORUM_CAT) {
				...
				// 'U_VIEWFORUM'			=> append_sid("{$phpbb_root_path}".$dir."viewforum.$phpEx", 'f=' . $row['forum_id']))	// CC Removed 12/30/10
				);
			...
			if ($row['forum_status'] == ITEM_LOCKED) {
				...
				//$folder_alt = ($forum_unread) ? 'NEW_POSTS' : 'NO_NEW_POSTS';	// CC Removed 12/30/10
...
//tsn.7 Change -- added ".$dir." // CC Removed 12/30/10
				//$last_post_url = append_sid("{$phpbb_root_path}".$dir."viewtopic.$phpEx", 'f=' . $row['forum_id_last_post'] . '&amp;p=' . $row['forum_last_post_id']) . '#p' . $row['forum_last_post_id'];
...
//tsn.7 Change -- added ".$dir."	// CC Removed 12/30/10
				//$s_subforums_list[] = '<a href="'. $dir . $subforum['link'] . '" class="subforum ' . (($subforum['unread']) ? 'unread' : 'read') . '">' . $subforum['name'] . '</a>';
			...
			if ($row['forum_type'] != FORUM_LINK) {
//tsn.7 Change -- added ".$dir."	// CC Removed 12/30/10
				//$u_viewforum = append_sid("{$phpbb_root_path}".$dir."viewforum.$phpEx", 'f=' . $row['forum_id']);
				$u_viewforum = append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $row['forum_id']);
			} else {
