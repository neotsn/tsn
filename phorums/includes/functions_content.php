...
function get_username_string(
	...
	$username_title = $username;
			...
			case 'avatar':
				...
				if ($mode == 'avatar') {
					...
					if (!empty($subAvatar['user_avatar'])) {
						$username = get_user_avatar($subAvatar['user_avatar'], $subAvatar['user_avatar_type'],
							$subAvatar['user_avatar_width'], $subAvatar['user_avatar_height'], $username, false,
							$subAvatar['user_avatar_max_dim']);
						if ($user_id && $user_id != ANONYMOUS && ($user->data['user_id'] == ANONYMOUS || $auth->acl_get('u_viewprofile'))) {
							$profile_url = ($custom_profile_url !== false) ? $custom_profile_url . '&amp;u=' . (int)$user_id : str_replace(array(
								'={USER_ID}',
								'=%7BUSER_ID%7D'
							), '=' . (int)$user_id, $_profile_cache['base_url']);
						} else {
							$profile_url = '';
						}
					}
		...
		if (($mode == 'full' && !$profile_url) || $mode == 'no_profile') {
			return str_replace(array('{USERNAME_COLOUR}', '{USERNAME}', '{USERNAME_TITLE}'),
				array($username_colour, $username, $username_title),
				(!$username_colour) ? $_profile_cache['tpl_noprofile'] : $_profile_cache['tpl_noprofile_colour']);
		}

	return str_replace(array('{PROFILE_URL}', '{USERNAME_COLOUR}', '{USERNAME}', '{USERNAME_TITLE}'),
		array($profile_url, $username_colour, $username, $username_title),
