 ...
 *
 * @param string      $mode               Can be profile (for getting an url to the profile), username (for obtaining the username), colour (for obtaining the user colour), full (for obtaining a html string representing a coloured link to the users profile) or no_profile (the same as full but forcing no profile link), or avatar (showing the avatar instead of user info)
 ...
 *
 * @param array       $subAvatar          Substitute the avatar for the user's name, supplying an array of avatar, type, width, height, and dimension override
 *
...
function get_username_string(
    ...
    $subAvatar = array('user_avatar_max_dim' => 25)
) {
    ...
    if (empty($_profile_cache)) {
        ...
        $_profile_cache['tpl_noprofile_colour'] = '<span style="color: {USERNAME_COLOUR};" class="username-coloured" title="{USERNAME_TITLE}">{USERNAME}</span>';
        $_profile_cache['tpl_profile'] = '<a href="{PROFILE_URL}" title="{USERNAME_TITLE}">{USERNAME}</a>';
        $_profile_cache['tpl_profile_colour'] = '<a href="{PROFILE_URL}" title="{USERNAME_TITLE}" style="color: {USERNAME_COLOUR};" class="username-coloured">{USERNAME}</a>';
    }
    ...
    switch ($mode) {
        ...
        case 'avatar':
            ...
            // Build the avatar and swap in for the username
            if ($mode == 'avatar') {
                if ($user_id && empty($subAvatar['user_avatar'])) {
                    global $db;
                    ...
                    $result = $db->sql_query($sql);
                    $subAvatar = array_merge($db->sql_fetchrow($result), $subAvatar);
                }

                if (!empty($subAvatar['user_avatar'])) {
                    $username = get_user_avatar($subAvatar['user_avatar'], $subAvatar['user_avatar_type'], $subAvatar['user_avatar_width'], $subAvatar['user_avatar_height'], $username, false, $subAvatar['user_avatar_max_dim']);
                    if (!$username && !empty($subAvatar['user_avatar'])) {
                        $username = '<span class="tsn8_default_avatar_mini"></span>';
                    }
                }
    ...
    if (($mode == 'full' && !$profile_url) || $mode == 'no_profile') {
        return str_replace(array('{USERNAME_COLOUR}', '{USERNAME}', '{USERNAME_TITLE}'), array($username_colour, $username, $username_title), (!$username_colour) ? $_profile_cache['tpl_noprofile'] : $_profile_cache['tpl_noprofile_colour']);
    } else {
        return str_replace(array('{PROFILE_URL}', '{USERNAME_COLOUR}', '{USERNAME}', '{USERNAME_TITLE}'), array($profile_url, $username_colour, $username, $username_title), (!$username_colour) ? $_profile_cache['tpl_profile'] : $_profile_cache['tpl_profile_colour']);
    }
}
