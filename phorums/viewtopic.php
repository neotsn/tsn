...
$topic_mod = ($auth->acl_get('m_', $forum_id)) ? '<option value="" selected="selected">'.$user->lang['QUICK_MOD'].'</option>' : '';
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i) {
    ...
    $postrow = array(
        ...
        'ONLINE_IMG'           => ($poster_id == ANONYMOUS || !$config['load_onlinetrack']) ? '' : (($user_cache[$poster_id]['online']) ? 'online' : 'offline'),
