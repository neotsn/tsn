...
if ($keywords || $author || $author_id || $search_id || $submit) {
    ...
    $sql = 'SELECT f.forum_id, f.forum_name, f.parent_id, f.forum_type, f.right_id, f.forum_password, fa.user_id FROM ' . FORUMS_TABLE . ' f LEFT JOIN ' . FORUMS_ACCESS_TABLE . " fa ON (fa.forum_id = f.forum_id AND fa.session_id = '" . $db->sql_escape($user->session_id) . "') $not_in_fid ORDER BY f.left_id";
...
//        $l_search_title = $user->lang['SEARCH_NEW'];
    ...
    if ($sql_where) {
        ...
        $sql = "SELECT $sql_select FROM $sql_from WHERE $sql_where ORDER BY " . $sort_by_sql[$sort_key] . " " . (($sort_dir == 'd') ? "DESC" : "ASC");
