<?php
	...
	if ($keywords || $author || $author_id || $search_id || $submit) {
		...
		if ($search_id) {
			$l_search_title = $user->lang['MYSPOT'];
			// force sorting
		...
		if ($sql_where) {
			//u.user_avatar, u.user_avatar_type, u.user_avatar_width, u.user_avatar_height //USERS_TABLE . ' u, ' .
			$sql_from = USERS_TABLE . ' u, ' . POSTS_TABLE . ' p, ' . TOPICS_TABLE . ' t
				LEFT JOIN ' . FORUMS_TABLE . ' f ON (f.forum_id = t.forum_id)' .
				(($sort_key == 'a') ? ' LEFT JOIN ' . USERS_TABLE . ' u ON (u.user_id = t.topic_poster) ' : '');
			$sql_select = 't.*, f.forum_id, f.forum_name, u.user_avatar, u.user_avatar_type, u.user_avatar_width, u.user_avatar_height, p.post_text, p.bbcode_uid, p.bbcode_bitfield';
			$sql_where .= ' AND u.user_id = t.topic_last_poster_id AND p.post_id = t.topic_last_post_id';

			if ($user->data['is_registered']) {
				if ($config['load_db_track']) {
					$sql_from .= '
						LEFT JOIN ' . TOPICS_POSTED_TABLE . ' tp ON (tp.user_id = ' . $user->data['user_id'] . '
				...
				if ($config['load_db_lastread']) {
					$sql_from .= '
					 LEFT JOIN ' . TOPICS_TRACK_TABLE . ' tt ON (tt.user_id = ' . $user->data['user_id'] . '
			...
			foreach ($rowset as $row) {
				...
				$topic_title = (strlen($topic_title) > 33) ? smart_excerpt($topic_title, 30) . '...' : smart_excerpt($topic_title);
				...
				if ($show_results == 'topics') {
//					$avatar_image = get_user_avatar($row['user_avatar'], $row['user_avatar_type'], $row['user_avatar_width'], $row['user_avatar_height']);
					...
					// Parse the message
					$message = generate_text_for_display($row['post_text'], $row['bbcode_uid'], $row['bbcode_bitfield'], 1);
					$message = (strlen($message) > $return_chars) ? smart_excerpt($message, 140) . ' ...' : smart_excerpt($message);
					...
					$tpl_ary = array(
						...
						'LAST_POST_AUTHOR_AVATAR' => get_user_avatar($row['user_avatar'], $row['user_avatar_type'], $row['user_avatar_width'] * 0.75, $row['user_avatar_height'] * 0.75),
